export const validateAndSubmitAuth = ({
        isLogin, username, email, password, confirmPassword,setLocalError, handleLogin, handleRegister
    }) => {
        setLocalError(null);
        ...
        if (!isLogin && password !== confirmPassword) return setLocalError("Passwords do not match");
        ...
        if (!isLogin) {
            if (username.length < 3) return setLocalError("Username must be at least 3 characters long");
