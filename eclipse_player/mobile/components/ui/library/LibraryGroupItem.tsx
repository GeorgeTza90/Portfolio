          ...
          {!isArtist && (
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <CollectionCard
                            songItem={item}
                            onPress={() => router.push(`/library/CollectionDetail?album=${encodeURIComponent(item.album)}`)}
                        />
                    )}
          ...
          {isArtist && (
            <FlatList
                ...
                renderItem={({ item }) => (
                    <CollectionCard
                        artistItem={item}
                        onPress={() => router.push(`/library/ArtistInfo?artist=${encodeURIComponent(item.name)}`)}
                    />
                )}
                initialNumToRender={5}
                maxToRenderPerBatch={10}
