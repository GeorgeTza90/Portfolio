...
export default function PlaylistDetail() {
    ...
    const handleDragEnd = async ({ data, to }: any) => {
        ...
        if (!movedSong) return;
        try {
            await putCall( "moveSongInPlaylist", id, Number(movedSong.playlistSongId), to );
        } catch (err) {
            setSongs(previous);
    ...
    return (
        <LinearGradient colors={[ hexToRgba("#000000be", 0.8), "#13131645" ]} style={styles.container}>
...
const styles = StyleSheet.create({
    container: { flex: 1, borderRadius: 25, padding: 10, margin: 15 },
